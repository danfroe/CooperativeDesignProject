...
from BenzenePlantWithRecycle_CompressorEnergyBalance import T_outCompressor
...
T_stream2vapors = fsolve(solveTemperatureOfMixedStreams_vapor, 300, \
                         args=(np.array([T_outCompressor, sd.T_stream12]), np.array([sd.nHydrogen_stream11, sd.nHydrogen_stream12]), np.array([sd.nMethane_stream11, 0]), 0, 0))
...
#--------- 
nHydrogen_in = np.array([0, sd.nHydrogen_stream9, sd.nHydrogen_stream11, sd.nHydrogen_stream12])
nMethane_in = np.array([0, sd.nMethane_stream9, sd.nMethane_stream11, 0])
nToluene_in = np.array([sd.nToluene_stream1, sd.nToluene_stream9, sd.nToluene_stream11, 0])
T_in = np.array([sd.T_stream1, sd.T_stream9, T_outCompressor, sd.T_stream12])
T_stream2 = fsolve(solveTemperatureOfMixedStreams_vapor, 300, args=(T_in,nHydrogen_in,nMethane_in,0,nToluene_in))
#---------
massflow_stream2vapors = MassflowProcessfluid(sd.nHydrogen_stream11+sd.nHydrogen_stream12, sd.nMethane_stream11, 0, 0)
...
Q_E101vapors = quad(integrand_heatEnthalpyProcessFluid_vapor,T_stream2vapors[0],sd.T_stream3, \
               args=(massflow_stream2vapors, sd.nHydrogen_stream11+sd.nHydrogen_stream12, sd.nMethane_stream11, 0, 0))

Cp_Toluene_liquid = heatcapacity_interpolation((T_stream2liquids[0]+Tboil_Toluene)/2, 150+273.15, 200+273.15, Cp_Toluene_liquid150C, Cp_Toluene_liquid200C)
dhv_Toluene = dhv_WatsonsEquation(150+273.15, Tboil_Toluene, T_crit_Toluene, dhv_Toluene150C)
Q_E101liquids = massflow_stream2liquids * Cp_Toluene_liquid * (Tboil_Toluene - T_stream2liquids[0]) \
...
#Cp_Benzene_liquid_recycle = heatcapacity_interpolation(sd.T_condenser_out, 100+273.15, 150+273.15, Cp_Benzene_liquid100C, Cp_Benzene_liquid150C)
...
#                          args=(np.array([sd.T_stream7, sd.T_condenser_out]), np.array([sd.nBenzene_stream7, sd.nBenzene_recycle]), \
