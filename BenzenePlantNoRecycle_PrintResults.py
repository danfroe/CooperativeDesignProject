...
import BenzenePlantNoRecycle_HXEnergyBalances as hx
import BenzenePlantNoRecycle_HeaterEnergyBalances as heater
import BenzenePlantNoRecycle_MechanicalEnergyBalances as pump
import pandas as pd
import openpyxl
...
columns = ['P1', 'P2']
index = ['p in [bar]', 'p out [bar]', 'height [m]', 'Mass flow [kg/h]', 'Shaft work [kW]', 'efficiency']
p_in = [sd.p_beforeToluenePump*10**-5, 3]
p_out = [sd.p_afterToluenePump*10**-5, 3]
height = [0, sd.deltaHeight_RefluxPump]
massflow = [pump.massflow_ToluenePump, pump.massflow_RefluxPump]
Work = [pump.W_P101[0], pump.W_P102]
efficiency = [sd.efficiency_ToluenePump, sd.efficiency_RefluxPump]

data_frame_2 = pd.DataFrame([p_in, p_out, height, massflow, Work, efficiency],
                  index=index, columns=columns)


with pd.ExcelWriter('EnergyBalances_NoRecycle.xlsx') as writer:
    data_frame_1.to_excel(writer, sheet_name='Heat Exchange without Recycle')
    data_frame_2.to_excel(writer, sheet_name='Shaft Work without Recycle')